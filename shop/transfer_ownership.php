        ...
        if ($item['status'] == 'stolen') {
            ...
            if ($check_user->num_rows > 0) {
                ...
                if ($new_owner['role'] == 'customer') {
                    ...
                    // Update ownership and status to sold
                    $sql = "UPDATE jewellery SET current_owner_id = $new_owner_id, status = 'sold' WHERE token_id = $token_id";
                    if ($conn->query($sql) === TRUE) {
                        ...
                        log_action($conn, $user_id, 'shop', 'transfer_jewellery', "Sold jewellery (Token ID: $token_id) to customer (ID: $new_owner_id)");
                        $success = "Jewellery sold to Customer successfully!";
                        
                    } else {
                        $error = "Error updating ownership.";
                        log_action($conn, $user_id, 'shop', 'failed_transfer', "Error updating ownership for token ID $token_id: " . $conn->error);
                    }
                } else {
                    $error = "The recipient must be a registered Customer.";
                }
            } else {
                $error = "Customer email not found. They must register first.";
            }
        }
    } else {
...
<div class="card" style="max-width: 500px; margin: 2rem auto;">
    <h2 style="color: var(--primary-gold); margin-bottom: 2rem;">Sell / Transfer to Customer</h2>
    ...
    <form method="POST">
        ...
        <div class="form-group">
            ...
            <input type="email" name="new_owner_email" class="form-control" required placeholder="Enter customer's email">
        </div>
        <button type="submit" class="btn btn-gold" style="width: 100%; margin-top: 1rem;">Complete Sale & Sign</button>
    </form>
</div>
